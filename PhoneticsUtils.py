...
def check_phonetics(word):
    if word.lower() in GzzCMUDict:
        return GzzCMUDict[word.lower()]
    return []
...
class cmusyllables(object):
    ...
    def SyllableCount(self, AszWord, AszMode = 'max', AlgFallBack=True):
        ...
        if len(LszWord) == 0 or LszWord not in GzzCMUDict:
            ...
            LliSyllableList = SyllableCount(LszWord)#self.dcSyllableCount[LszWord]
            self.szMode = 'lookup'
...
def count_word_syllables(word):
    ...
    return LzzSyllableCounter.SyllableCount(word)
